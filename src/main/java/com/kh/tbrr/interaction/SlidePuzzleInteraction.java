 ...
 * (ステートレス設計：状態はSlidePuzzleSessionで管理)
 */
public class SlidePuzzleInteraction implements InteractionHandler {
    ...
    public CompletableFuture<InteractionResult> execute(Map<String, Object> params, Player player) {
        // パズルの実行状態（セッション）を新規作成して開始
        SlidePuzzleSession session = new SlidePuzzleSession(params);
        return session.start();
    }

    /**
     * パズルの1回分の実行状態を管理するセッションクラス
     * （シングルトンのハンドラ本体に状態を残さないための使い捨てクラス）
     */
    private static class SlidePuzzleSession {
        ...
        public SlidePuzzleSession(Map<String, Object> params) {
            this.params = params;
            ...
            // パラメータから設定を取得
            this.maxMoves = 100; // デフォルト最大手数
            this.timeLimit = 120; // デフォルト制限時間（秒）
            
            if (params.containsKey("maxMoves")) {
                Object val = params.get("maxMoves");
                if (val instanceof Number) {
                    this.maxMoves = ((Number) val).intValue();
                }
            }
            if (params.containsKey("timeLimit")) {
                Object val = params.get("timeLimit");
                if (val instanceof Number) {
                    this.timeLimit = ((Number) val).intValue();
                }
            }
        }

        public CompletableFuture<InteractionResult> start() {
            Platform.runLater(this::showPuzzleUI);
            return future;
        }
        ...
        private void showPuzzleUI() {
            ...
            currentTimer = new Timeline();
            currentTimer.setCycleCount(timeLimit);
            currentTimer.getKeyFrames().add(new KeyFrame(Duration.seconds(1), e -> {
                timeRemaining[0]--;
